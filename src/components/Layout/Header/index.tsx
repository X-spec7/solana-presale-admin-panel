...
import { CustomButton, SmoothScrollLink } from "@/components/Common";
...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header left-0 top-0 z-40 flex w-full items-center border-b border-gray-400 dark:border-gray-700 ${
          sticky
          ...
          <div className="relative -mx-4 flex items-center justify-between">
                  ...
                  {menuData.map((menuItem, index) => (
                    <li key={index} className="group relative">
                      {menuItem.path ? (
                          ...
                          <Link
                            href={menuItem.path}
                            className={`flex py-2 text-base lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 text-dark hover:text-primary dark:text-white/70 dark:hover:text-white ${usePathName === menuItem.path
            ...
            <div className="flex items-center justify-end pr-16 lg:pr-0">
              {isLoggedIn ? (
                ...
                <div className="flex w-32 justify-between gap-4">
                  <CustomButton href="/" variant="outline" className="hidden md:inline-block mr-4">
                    Sign In
                  </CustomButton>
                  {/* <CustomButton href="/" variant="solid" className="hidden md:inline-block">
                    Sign Up
                  </CustomButton> */}
                </div>
              )}
              <div className="absolute right-16 sm:static">
                <ThemeToggler />
              </div>
            </div>
          </div>
